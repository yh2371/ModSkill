...
#from smpl_sim.mujoco_utils.utils import get_body_qposaddr
...
res_data = joblib.load("data/mdm/res_hand.pkl")

# ipdb.set_trace()
amass_data = {}
print(res_data.keys())
print(len(res_data['json_file']['thetas']))
joint_num = 24 #24 if smpl
for i in range(len(res_data['json_file']['thetas'])):
    pose_aa = np.array(res_data['json_file']['thetas'])[i].reshape(-1, joint_num, 3)
    print(pose_aa.shape)
    B = pose_aa.shape[0]
    trans = np.array(res_data['json_file']['root_translation'][i]).T

    pose_aa = pose_aa.reshape(B, joint_num*3)
    ...
    transform = sRot.from_euler('xyz', np.array([0, 0, 0]), degrees=False)
