...
if __name__ == '__main__':
    ...
    try:
        checkpoint = torch_ext.load_checkpoint(f"/home/yhuang/tracking/ModTrack/output/HumanoidIm/{exp_name}/Humanoid_{epoch:08d}.pth")
    except:
        print(os.path.exists(f"/home/yhuang/tracking/ModTrack/output/HumanoidIm/{exp_name}/Humanoid_{epoch:08d}.pth"))
    amass_train_data_take6 = joblib.load(args.data)

    failed_keys_dict = {}
    termination_history_dict = {}
    all_keys = set()
    for failed_path in sorted(glob.glob(f"/home/yhuang/tracking/ModTrack/output/HumanoidIm/{exp_name}/failed_*"))[:]:
