...
class HumanoidIm(humanoid_amp_task.HumanoidAMPTask):
    ...
    def get_task_obs_size(self):
        ...
        if (self._enable_task_obs):
            if self.obs_v == 1:
                ...
                print("obs_sizessssss666", obs_size, self._track_bodies, self._num_traj_samples)
                ...
                print("obs_sizessssss777", obs_size)
    ...
    def post_physics_step(self):
        ...
        if flags.im_eval:
            ...
            self.extras['jointwise_mpjpe'] = (body_pos - motion_res['rg_pos']).norm(dim=-1)
...
def compute_imitation_observations_v6(root_pos, root_rot, body_pos, body_rot, body_vel, body_ang_vel, ref_body_pos, ref_body_rot, ref_body_vel, ref_body_ang_vel, time_steps, upright):
    ...
    # CHANGE: JOINT_WISE ORDERING
    obs.append(diff_local_body_pos_flat.view(B, time_steps, J, -1))  # 1 * timestep * 24 * 3
    obs.append(torch_utils.quat_to_tan_norm(diff_local_body_rot_flat).view(B, time_steps, J, -1))  #  1 * timestep * 24 * 6
    obs.append(diff_local_vel.view(B, time_steps, J, -1))  # timestep  * 24 * 3
    obs.append(diff_local_ang_vel.view(B, time_steps, J, -1))  # timestep  * 24 * 3
    obs.append(local_ref_body_pos.view(B, time_steps, J, -1))  # timestep  * 24 * 3
    obs.append(local_ref_body_rot.view(B, time_steps, J, -1))  # timestep  * 24 * 6
...
def compute_humanoid_im_reset(reset_buf, progress_buf, contact_buf, contact_body_ids, rigid_body_pos, ref_body_pos, pass_time, enable_early_termination, termination_distance, disableCollision, use_mean):
    ...
    if (enable_early_termination):
        ...
        # first timestep can so
        # metimes still have nonzero contact forces
