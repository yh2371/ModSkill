...
class MotionLibSMPL(MotionLibBase):
    ...
    def __init__(self, motion_lib_cfg):
        ...
        if osp.exists(data_dir):
            ...
            #self.mesh_parsers = {0: smpl_parser_n, 1: smpl_parser_m, 2: smpl_parser_f}
                ...
                smpl_parser_n = SMPLX_Parser(model_path=data_dir, gender="neutral", use_pca = False)
                smpl_parser_m = SMPLX_Parser(model_path=data_dir, gender="male", use_pca = False)
                smpl_parser_f = SMPLX_Parser(model_path=data_dir, gender="female", use_pca = False)
            self.mesh_parsers = {0: smpl_parser_n, 1: smpl_parser_m, 2: smpl_parser_f}
        else:
    ...
    def fix_trans_height(pose_aa, trans, curr_gender_betas, mesh_parsers, fix_height_mode):
        ...
        with torch.no_grad():
            ...
            #for i in [pose_aa[:frame_check], betas[None,], trans[:frame_check]]:
            #    print(i.shape, "sup")
            vertices_curr, joints_curr = mesh_parser.get_joints_verts(pose_aa[:frame_check], betas[None,:10], trans[:frame_check])
