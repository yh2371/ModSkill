...
class MotionLibBase():
    ...
    def merge_generated_motions(self, generated_motions_dict):
        """
        Merge generated motions into the existing motion data.
        
        This method merges generated motions into the existing motion data structure,
        making them available for training in subsequent iterations.
        
        Args:
            generated_motions_dict: Dictionary mapping motion keys to motion data dicts
            
        Returns:
            Number of motions successfully merged
        """
        if not isinstance(self._motion_data_load, dict):
            print("Warning: Cannot merge generated motions - motion_data_load is not a dict")
            return 0
        
        # Merge the generated motions into the existing data
        merged_count = 0
        for key, motion_data in generated_motions_dict.items():
            if key not in self._motion_data_load:
                self._motion_data_load[key] = motion_data
                merged_count += 1
        
        if merged_count > 0:
            # Update the data list and keys
            self._motion_data_list = np.array(list(self._motion_data_load.values()))
            self._motion_data_keys = np.array(list(self._motion_data_load.keys()))
            old_num_unique = self._num_unique_motions
            self._num_unique_motions = len(self._motion_data_list)
            
            # Extend termination history and sampling prob for new motions
            old_num = len(self._termination_history)
            new_num = self._num_unique_motions
            if new_num > old_num:
                # Pad with zeros for new motions (they start with no termination history)
                padding = torch.zeros(new_num - old_num, device=self._device)
                self._termination_history = torch.cat([self._termination_history, padding])
                self._success_rate = torch.cat([self._success_rate, padding])
                self._sampling_history = torch.cat([self._sampling_history, padding])
                
                # Update sampling prob: keep existing probabilities normalized, new motions get equal share
                # Option 1: Uniform sampling across all motions
                self._sampling_prob = torch.ones(new_num, device=self._device) / new_num
                # Option 2: Keep existing probabilities and add new ones with equal weight
                # existing_weight = self._sampling_prob.sum()
                # new_weight = (new_num - old_num) / new_num
                # self._sampling_prob = torch.cat([
                #     self._sampling_prob * (1 - new_weight) / existing_weight if existing_weight > 0 else self._sampling_prob,
                #     torch.ones(new_num - old_num, device=self._device) * new_weight / (new_num - old_num)
                # ])
            
            print(f"Merged {merged_count} generated motions into motion library")
            print(f"Total motions: {old_num_unique} -> {self._num_unique_motions}")
            print("Note: Motion library needs to be reloaded (via load_motions) for new motions to be available for sampling")
