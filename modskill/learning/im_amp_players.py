...
class IMAMPPlayerContinuous(amp_players.AMPPlayerContinuous):
    ...
    def _post_step(self, info, done):
        ...
        if flags.im_eval:
            ...
            #print("CHECKUP!",info["jointwise_mpjpe"])
            ...
            if self.curr_stpes >= curr_max or self.terminate_state.sum() == humanoid_env.num_envs:
                #print("STEPS!",self.curr_stpes, curr_max, self.terminate_state)
                ...
                #for idx, i in enumerate(humanoid_env._motion_lib.get_motion_num_steps()):
                    #print(all_mpjpe[: (i - 1), idx].shape, all_mpjpe[: :, idx].shape)
                # all_mpjpe = [all_mpjpe[: , idx].mean() for idx, i in enumerate(humanoid_env._motion_lib.get_motion_num_steps())] # -1 since we do not count the first frame. 
                # all_body_pos_pred = np.stack(self.pred_pos)
                # all_body_pos_pred = [all_body_pos_pred[:, idx] for idx, i in enumerate(humanoid_env._motion_lib.get_motion_num_steps())]
                # all_body_pos_gt = np.stack(self.gt_pos)
                # all_body_pos_gt = [all_body_pos_gt[:, idx] for idx, i in enumerate(humanoid_env._motion_lib.get_motion_num_steps())]
                ...
                if (humanoid_env.start_idx + humanoid_env.num_envs >= humanoid_env._motion_lib._num_unique_motions):
                    ...
                    import pickle
                    pickle.dump([pred_pos_all, gt_pos_all, metrics],open("./modtrack_check_new.pkl", "wb"))
                    print("results saved.")
                    ...
                    pickle.dump(failed_keys, open("./failed_test_new.pkl", "wb"))
                    pickle.dump(success_keys, open("./long_succ_new.pkl", "wb"))
                    print("....dumped")
                    if COLLECT_Z:
                    ...
                    # joblib.dump(failed_keys, "./failed.pkl")

                    #joblib.dump(failed_keys, "./failed.pkl")
                    #joblib.dump(success_keys, "./long_succ.pkl")
                    print("....dumped")
                    #joblib.dump(success_keys, osp.join(self.config['network_path'], "long_succ.pkl"))
                    print("....")
                    import ipdb; ipdb.set_trace()
                    ...
                    if done_count > 0:
                        ...
                        if self.print_stats:
                            if print_game_res:
                                ...
                                #print(info['jointwise_mpjpe'].shape)
