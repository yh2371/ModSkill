...
from phc.learning.generative_adaptive_sampling import GenerativeAdaptiveSampling
...
class IMAmpAgent(amp_agent.AMPAgent):
    def __init__(self, base_name, config):
        ...
        # Initialize generative adaptive sampling at the start if enabled
        # This ensures MDM model is loaded and ready before training/evaluation
        # The MDM model initialization can take time, so doing it early avoids delays during evaluation
        self._gen_adaptive_sampling = None
        try:
            if hasattr(self.vec_env, 'env') and hasattr(self.vec_env.env, 'task'):
                humanoid_env = self.vec_env.env.task
                if hasattr(humanoid_env, 'auto_pmcp_generative') and humanoid_env.auto_pmcp_generative:
                    print("=" * 80)
                    print("Initializing Generative Adaptive Sampling at simulation start...")
                    print("=" * 80)
                    self._gen_adaptive_sampling = GenerativeAdaptiveSampling(
                        agent=self,
                        motion_lib=humanoid_env._motion_lib,
                        text_mapping_path=getattr(humanoid_env, 'text_mapping_path', None),
                        text_base_path=getattr(humanoid_env, 'text_base_path', None),
                        index_csv_path=getattr(humanoid_env, 'index_csv_path', None),
                        num_samples_per_failed=getattr(humanoid_env, 'gen_samples_per_failed', 3),
                        discriminator_threshold=getattr(humanoid_env, 'gen_discriminator_threshold', 0.5),
                        mdm_model_path=getattr(humanoid_env, 'mdm_model_path', None),
                        mdm_repo_path=getattr(humanoid_env, 'mdm_repo_path', None),
                        device=self.device
                    )
                    if self._gen_adaptive_sampling.mdm_talker is not None:
                        print("✓ Generative Adaptive Sampling initialized successfully - MDM model ready")
                    else:
                        print("⚠ Warning: Generative Adaptive Sampling initialized but MDM model not loaded")
                        print("  Please check mdm_model_path configuration")
                    print("=" * 80)
        except Exception as e:
            print(f"Warning: Could not initialize Generative Adaptive Sampling at startup: {e}")
            print("It will be initialized lazily when needed (if auto_pmcp_generative is enabled)")
            self._gen_adaptive_sampling = None
    ...
    def update_training_data(self, failed_keys):
        ...
        if humanoid_env.auto_pmcp:
        ...
        elif humanoid_env.auto_pmcp_generative:
            # Generative adaptive sampling
            print("=" * 80)
            print("Generative Adaptive Sampling: Processing failed cases")
            print("=" * 80)
            print(f"Processing {len(failed_keys)} failed cases with generative adaptive sampling")
            
            # Use pre-initialized generative adaptive sampling (initialized at simulation start)
            if self._gen_adaptive_sampling is None:
                print("ERROR: Generative adaptive sampling not initialized!")
                print("This should have been initialized at simulation start.")
            ...
            # Check if MDM model is ready
            if self._gen_adaptive_sampling.mdm_talker is None:
                print("ERROR: MDM model not loaded. Cannot generate motions.")
                print("Please check mdm_model_path configuration.")
            else:
                # Generate and evaluate motions
                stats = self._gen_adaptive_sampling.generate_and_add_motions(failed_keys)
                print(f"Generative sampling stats: {stats}")
                
                # Load and merge generated motions if any were saved
                generated_motions_path = osp.join(self.network_path, f"generated_motions_epoch_{self.epoch_num:010d}.pkl")
                if osp.exists(generated_motions_path):
                    generated_motions = joblib.load(generated_motions_path)
                    merged_count = humanoid_env._motion_lib.merge_generated_motions(generated_motions)
                    if merged_count > 0:
                        print(f"Merged {merged_count} generated motions into training data")
                        # Reload motions to make them available for sampling
                        # This will be done automatically on next training iteration via load_motions
                        # For immediate availability, we could call load_motions here, but it's expensive
                        print("Generated motions will be available in next training iteration")
                else:
                    print(f"No generated motions file found at {generated_motions_path}")
