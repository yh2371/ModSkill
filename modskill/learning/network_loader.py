...
def load_pnn(checkpoint, num_prim, has_lateral, activation = "relu", device = "cpu"):
    ...
    pnn = ModSkillNetwork(mlp_args, output_size=checkpoint['model']['a2c_network.mu.bias'].shape[0], numCols=num_prim, has_lateral=has_lateral)
    state_dict = pnn.state_dict()
    for k in state_dict_load.keys():
        if "modskill_network" in k:
            pnn_dict_key = k.split("modskill_network.")[1]
            print(pnn_dict_key, k)
